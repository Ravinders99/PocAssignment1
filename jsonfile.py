...
for prize in filtered_prizes:
    ...
    # Update the 'motivation' field for each laureate
    for laureate in prize.get("laureates", []):
        if "motivation" in laureate:
            # 1. Replace double double-quotes ("") with a single double-quote (")
            laureate["motivation"] = laureate["motivation"].replace('""', '"')
            
            # 2. Replace escaped quotes (e.g., \") with a single quote or a normal double-quote
            laureate["motivation"] = laureate["motivation"].replace('\\"', '"')
